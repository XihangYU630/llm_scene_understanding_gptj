...
class DataGenerator:
    ...
    def __init__(
        ...
        self.dataset = Matterport3dDataset("./mp_data/" + label_set +
                                           "_matterport3d_w_edge_502030_new.pkl")
    ...
    def generate_data(self, k, num_objs, data_folder, split, suffix, all_permutations=True, skip_rms=True):
        """
    ...
    def data_split_generator(self, data_generation_params, k_test, data_folder):
        ...
        if k_test > 0:
            ...
            sentences, all_objs_list, labels = dg.generate_data(
                k_test, k_test, data_folder, 'test', suffix, all_permutations=False, skip_rms=False)
            count += len(sentences)
            TEMP[suffix] = [
                sentences, all_objs_list, labels
            ]
            ...
            for use_gt in [True]:
                ...
                for split in splits:
                    for suffix in split_dict[split]:
                        sentences, all_objs_list, labels = split_dict[
                            split][suffix]
