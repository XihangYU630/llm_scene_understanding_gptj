...
from models import ContrastiveNet, FeedforwardNet_GPTJ
...
def train_job(lm, label_set, epochs, batch_size, co_suffix="", seed=0):
    ...
    embedding_size = 256
    query_net = FeedforwardNet_GPTJ(4096, embedding_size)  # TODO: Try other values
    label_net = FeedforwardNet_GPTJ(4096, embedding_size)  # TODO: Try other values
    query_net.to(device)
    label_net.to(device)
    ...
    with trange(epochs) as pbar:
        for epoch in pbar:
            ...
            for batch_idx, (query_em, _, label) in enumerate(train_dl):
                ...
                label_embeddings = label_embeddings.to(torch.float32)
